library(ggplot2)
coverage=read.table("iso1_HiFi_2R_coverage.bedgraph", sep="\t", header=F)
...
p <- ggplot(coverage,aes(x = locus1, y = depth)) + geom_point(size=0.01,color="coral2",alpha = 0.05) + geom_smooth(method="gam")  + labs(title = "Depth") + xlab("iso1-2R")  +  ylim(0, 200) + theme(aspect.ratio=0.3)
...
ggsave(filename="iso1_adults.2R.jpg",plot = print(p))
