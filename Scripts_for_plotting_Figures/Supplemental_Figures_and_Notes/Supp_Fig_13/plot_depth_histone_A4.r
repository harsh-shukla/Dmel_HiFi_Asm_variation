...
coverage1=read.table("A4_default.depth", sep="\t", header=F)
colnames(coverage1) <- c("Chr","locus","depth")
coverage1$locus <- coverage1$locus - coverage1$locus[1]

coverage2=read.table("A4_targeted.depth", sep="\t", header=F)
...
p <- p +  labs(title = "A4 Histone") + xlab("Histone Locus (kb)")  +  ylim(0, 200) + theme(aspect.ratio=0.3) 
#print(p)

p <- p + scale_x_continuous(breaks=seq(0,650000,50000),labels = c(0,"50","100","150","200","250","300","350","400","450","500","550","600","650"))

ggsave(filename="A4_together.F.jpg",plot = print(p),dpi = 600)
dev.off()
